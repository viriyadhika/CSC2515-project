...
def train_cnn_from_datasets(
    ...
    """
    Generic entry point to train the CNN on arbitrary 1D signals.

    Datasets are expected to return keys "input" and "labels". This
    works for ECG, audio, or any other 1D time series.
    """
...
def main():
    ...
    train_dataset = ECGDataset(X_train_t, y_train_t)
    valid_dataset = ECGDataset(X_valid_t, y_valid_t)
    test_dataset = ECGDataset(X_test_t, y_test_t)
    ...
    trainer, val_metrics, test_metrics = train_cnn_from_datasets(
        model=model,
        train_dataset=train_dataset,
