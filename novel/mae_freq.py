...
"""
Shared utilities for MAE-style ECG models.

This module intentionally has **no** heavy dependencies on the training scripts,
so it can be imported from both `novel/mae.py` (time-domain MAE) and
`novel/mae_freq.py` (STFT / spectrogram MAE).
"""
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./tiny_ecg_mae_freq_runs")
    ...
    # MAE pretraining uses only the beats (RR/labels are unused here)
